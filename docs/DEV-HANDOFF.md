...
### Phase 3 — 移动端适配 ✅
- 以 375px 为基准适配移动端，`viewport-fit=cover` 适配刘海屏
...
- **全局布局基线**：
  ...
  - `index.css` 新增 `surface-card` / `surface-muted` / `focus-ring` 设计基线
- **首页（`HomePage`）**：从移动单列扩展为桌面双栏（左内容 + 右城市预览网格）
...
| 检查项 | 验收标准 |
|--------|---------|
| 首页桌面布局（`>=1024px`）| 左右双栏，右侧为城市预览模块，不是单卡放大 |
| 问卷页桌面布局（`>=1024px`）| 双栏结构（左导航 + 中主区），无“当前匹配领先/当前选择”侧栏 |
| 结果页桌面布局（`>=1024px`）| 主结果与侧栏信息同屏可见，信息层级明确 |
| 宽屏可读性（`>=1280px`）| 不出现 `480px` 窄列居中效果 |
| 键盘可达性 | Tab 可访问关键按钮与题号导航，`focus-visible` 明确可见 |
| 移动端回归（375/390/430）| 流程、按钮触达、截图区域与此前一致，无新增横向滚动 |
| 多语言回归 | `/en` `/zh` `/ja` `/ko` 三页布局与文案均正常，无占位符 |
